...
function cacheMessage(cacheKey: string, entry: { message?: Message; fetched: boolean; }) {
    if (!messageCache.has(cacheKey) && messageCache.size >= 250) {
...
async function fetchMessage(channelId: string, messageId: string) {
    ...
    cacheMessage(cacheKey, { fetched: false });
...
function MessageEmbedAccessory({ message }: { message: Message; }) {
    ...
    for (const [_, channelId, messageId] of message.content!.matchAll(messageLinkRegex)) {
        ...
        if (!linkedMessage) {
            ...
            if (linkedMessage) {
                cacheMessage(cacheKey, { message: linkedMessage, fetched: true });
            } else {
